...
const GuildPerson = (props: GuildPersonProps) => {
  const [data, setData] = useState([
    { member_id: 1, name: 'name1', solve_percent: 12.5, difficulty: 2, join_at: '2024-01-31', thumbnail: 'https://picsum.photos/300' },
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap' }}>
        {data.map((value) => (
          <div style={{ width: '25%' }}>
            <MemberInfoCard member_id={value.member_id} name={value.name} thumbnail={value.thumbnail} solve_percent={value.solve_percent} difficulty={value.difficulty} join_at={value.join_at} />
          </div>
        ))}
      </div>
    </>
