      ...
      <div style={{ display: 'flex', flexWrap: 'wrap' }}>
        {data.map((value) => (
          <div style={{ display: 'flex', flexDirection: 'column', width: '25%', alignItems: 'center' }}>
            <MemberInfoCard key={value.memberId} member_id={value.memberId} name={value.name} thumbnail={value.thumbnail} difficulty={value.difficulty} join_at={value.joinAt} in_guild={true} />
            <Spacer space={'1vmin'} />
          </div>
        ))}
      </div>
