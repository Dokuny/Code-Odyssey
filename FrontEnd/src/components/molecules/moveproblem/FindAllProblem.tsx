...
const FindAllProblem: React.FC<Props> = ({ leftlist,checkHandler,checkedList }) => {
  ...
  return (
    ...
    <div style={{ width: '80%' ,margin: 'auto', marginTop : '10px' }}>
      <BasicInput placeholder={'문제를 찾아보기'} setInput={setSearchInput} input={''}                
              />
    </div>
    {leftlist.map(item => <ProblemSprintCard item={item} checkedList={checkedList} checkHandler={checkHandler} />)}
    </div>  
  );
};
